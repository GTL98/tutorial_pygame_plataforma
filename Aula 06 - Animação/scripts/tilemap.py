...
class Tilemap:
    ...
    def renderizar(self, superficie: object, offset=(0, 0)) -> None:
        ...
        # --- Renderizar somente os tiles que aparecem na tela --- #
        for x in range(offset[0] // self.tile_tamanho, (offset[0] + superficie.get_width()) // self.tile_tamanho + 1 ):
            for y in range(offset[1] // self.tile_tamanho, (offset[1] + superficie.get_width()) // self.tile_tamanho + 1):
                localizacao = f'{x};{y}'
                if localizacao in self.tilemap:
                    tile = self.tilemap[localizacao]
                    superficie.blit(
                        self.jogo.assets[tile['type']][tile['variant']],
                        (tile['pos'][0] * self.tile_tamanho - offset[0], tile['pos'][1] * self.tile_tamanho - offset[1])
                    )
